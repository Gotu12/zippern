...
/**
 * Fan-out PK guest camera + mic to live spectators on
 * `live_publishers/{liveSessionId}/{guestPublisherUid}/`, mirroring [WebRTCManager] broadcast host
 * semantics. Reuses the PK [PeerConnectionFactory] and local A/V tracks (no second camera).
 */
private open class FanoutSdpObserver : SdpObserver {
...
class PkGuestSpectatorFanout(
    ...
    fun start() {
        if (stopped) return
        Log.d(tag, "start live_publishers/$liveSessionId/$guestPublisherUid")
        ...
        val listener = object : ChildEventListener {
            override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                ...
                if (viewerId.isEmpty() || viewerId == guestPublisherUid) return
    ...
    fun stop() {
        ...
        Log.d(tag, "stopped live_publishers/$liveSessionId/$guestPublisherUid")
    }
    ...
    private fun spawnForViewer(viewerId: String) {
        ...
        val pc = factory.createPeerConnection(rtcConfig, object : PeerConnection.Observer {
            ...
            override fun onAddTrack(receiver: RtpReceiver?, p1: Array<out org.webrtc.MediaStream>?) {}
        }) ?: run {
        ...
        runCatching {
            pc.addTrack(localVideoTrack, listOf("pk_guest_main"))
            pc.addTrack(localAudioTrack, listOf("pk_guest_main"))
        ...
        val ansListener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                onMain {
                    ...
                    val conn = cur.peerConnection ?: return@onMain
                    conn.setRemoteDescription(object : FanoutSdpObserver() {
                        override fun onSetSuccess() {
                            onMain {
